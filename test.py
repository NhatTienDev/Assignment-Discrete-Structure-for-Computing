...
df = pd.read_csv('D:\Assignment Discrete Structure for Computing\VN Index Historical Data.csv', index_col = 'Date', parse_dates = True)
df = df.dropna()
...
df = pd.DataFrame({'Actual value': pd.Series(y_test, index = df['Date'].loc['1/4/22':'3/31/22']), 'Predicted value': pd.Series(y_predict, index = df['Date'].loc['1/4/22':'3/31/22'])})
...
plt.title('Price for the next 3 months of VNINDEX')
plt.xlabel('Index')
